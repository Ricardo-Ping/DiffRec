...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='E:/Model/DiffRec/datasets/beauty/', help='load data path')
    parser.add_argument('--lr', type=float, default=0.0001, help='learning rate')
    parser.add_argument('--weight_decay', type=float, default=0.0)
    parser.add_argument('--batch_size', type=int, default=400)
    parser.add_argument('--epochs', type=int, default=1000, help='upper epoch limit')
    parser.add_argument('--topN', type=str, default='[5, 10, 20]')
    ...
    param_num = 0
    mlp_num = sum([param.nelement() for param in model.parameters()])  # 计算MLP模型的参数数量
    diff_num = sum([param.nelement() for param in diffusion.parameters()])  # 扩散模型的参数数量（通常为0）
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            batch_count += 1
            optimizer.zero_grad()
            losses = diffusion.training_losses(model, batch, args.reweight)  # 计算损失
        ...
        if epoch % 5 == 0:
            ...
            evaluate_utils.print_results(None, valid_results, test_results, eval(args.topN))
            ...
            if valid_results[1][1] > best_recall:  # recall@20 as selection
                ...
                torch.save(model,
                           ...
                           .format(args.save_path, args.dataset, args.lr, args.weight_decay, args.batch_size, args.dims,
                                   args.emb_size, args.mean_type, args.steps, args.noise_scale, args.noise_min,
                                   args.noise_max, args.sampling_steps,
    ...
    evaluate_utils.print_results(None, best_results, best_test_results, eval(args.topN))
